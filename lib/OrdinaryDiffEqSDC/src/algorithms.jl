...
Fixed step size only: pass `adaptive = false` and a `dt`.",
    ...
    - `node_type`: node distribution, `:Legendre` or `:Equidistant`.
    - `quad_type`: which interval endpoints are nodes, `:Gauss` (neither),
        `:RadauLeft` (left), `:RadauRight` (right) or `:Lobatto` (both).
    - `num_sweeps`: number of sweeps `K` per step.
    - `sweeper`: the preconditioner `QΔ`. `:BE` (implicit Euler between the
        nodes), `:FE` (explicit Euler between the nodes), `:Trapezoid`, `:LU`
    ...
    - `step_update`: how the step solution is formed from the node values,
        `:quadrature` (`u_n + Δt Σ_m w_m f_m`) or `:lastnode` (`u_M`, which
        requires the last node to be the right endpoint).
    """,
    """
    num_nodes = 3,
    node_type = :Legendre,
    quad_type = :RadauRight,
    num_sweeps = 3,
    sweeper = :BE,
    step_update = :quadrature,
    """
)
...
function SDC(;
        num_nodes::Int = 3,
        node_type::Symbol = :Legendre,
        quad_type::Symbol = :RadauRight,
        num_sweeps::Int = 3,
        sweeper::Symbol = :BE,
        step_update::Symbol = :quadrature,
